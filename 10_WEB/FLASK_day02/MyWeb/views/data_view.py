...
def save_data() :
    ...
    if (request.method == 'GET') : 
        ...
        save_dir = 'C:/Users/KDP-48/PythonProject/Data/KDT-HTML/FLASK_day02/MyWeb/static/img'
        file.save(save_dir+'/'+file.filename)

        #return f"SAVE POST DATA =><br><br> METHOD : {method} <br><br> HEADERS : {headers} <br><br> ARGS :{args} <br>VALUE : {v}<br>MSG : {m}<br>filename : {file}"
        return f"filename : {file.filename}<br>filepath {save_dir}"
